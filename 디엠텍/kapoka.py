...
def crawl_category(url):
    ...
    for page in range(max_page):
        ...
        cate_link = []
        # 상품 목록 가져오기
        product_elements = driver.find_elements(By.XPATH, '/html/body/div[7]/div[1]/div/div[2]/div[1]/div[2]/ul/li')
        time.sleep(0.25)
        for element in product_elements:
            print(page)
            product_url = element.find_element(By.XPATH, './/a').get_attribute("href")
            # print(product_url)
...
def crawl_product(url):
    ...
    title = driver.find_element(By.XPATH, "/html/body/div[7]/div/div/div[2]/div[1]/div[1]/div[2]/div[2]/div[1]/table/tbody/tr[1]/td/span").text
    ...
    #     price = 4500
    #     price2 = int(price)*1.8
    img_url = driver.find_element(By.XPATH, "/html/body/div[7]/div/div/div[2]/div[1]/div[1]/div[1]/div[1]/div/a/img").get_attribute("src")
    print(img_url)
    # 상세 설명 (HTML 형식)
    detail_description = driver.find_element(By.CSS_SELECTOR, "#prdDetail > div > center").get_attribute('innerHTML')
    detail_description = re.sub(r'<!--.*?-->', '', detail_description, flags=re.DOTALL).replace(' src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR4nGP6zwAAAgcBApocMXEAAAAASUVORK5CYII="','').replace('ec-data-src','src')
...
def main():
    ...
    category_url = "https://dmteckb2b.com/product/list.html?cate_no=44"
    cate_list = ['033021']
    cate_name = ['키케이스']
    mall='디엠텍'
